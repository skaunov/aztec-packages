...
template <typename FF_> class ECCVMLookupRelationImpl {
  public:
    ...
    {
        return (in.msm_add[row] == 1) || (in.msm_skew[row] == 1) || (in.precompute_select[row] == 1);
    }
    ...
    static Accumulator compute_write_term(const AllEntities& in, size_t row, const Parameters& params)
    {
        ...
        // negative points map pc, round, x, -y
        // positive points map pc, 15 - (round * 2), x, y
        const auto& precompute_pc = View(in.precompute_pc[row]);
        const auto& tx = View(in.precompute_tx[row]);
        const auto& ty = View(in.precompute_ty[row]);
        const auto& precompute_round = View(in.precompute_round[row]);
        const auto& gamma = params.gamma;
        const auto& beta = params.beta;
        const auto& beta_sqr = params.beta_sqr;
        const auto& beta_cube = params.beta_cube;
    ...
    static Accumulator compute_read_term(const AllEntities& in, size_t row, const Parameters& params)
    {
        ...
        const auto& beta_sqr = params.beta_sqr;
        const auto& beta_cube = params.beta_cube;
        const auto& msm_pc = View(in.msm_pc[row]);
        const auto& msm_count = View(in.msm_count[row]);
        const auto& msm_slice1 = View(in.msm_slice1[row]);
        const auto& msm_slice2 = View(in.msm_slice2[row]);
        const auto& msm_slice3 = View(in.msm_slice3[row]);
        const auto& msm_slice4 = View(in.msm_slice4[row]);
        const auto& msm_x1 = View(in.msm_x1[row]);
        const auto& msm_x2 = View(in.msm_x2[row]);
        const auto& msm_x3 = View(in.msm_x3[row]);
        const auto& msm_x4 = View(in.msm_x4[row]);
        const auto& msm_y1 = View(in.msm_y1[row]);
        const auto& msm_y2 = View(in.msm_y2[row]);
        const auto& msm_y3 = View(in.msm_y3[row]);
        const auto& msm_y4 = View(in.msm_y4[row]);
