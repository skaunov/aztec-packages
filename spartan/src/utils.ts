 ...
 */
export function convertToMultiaddr(
  address: string,
  protocol: "tcp" | "udp"
): string {
  ...
  if (addr.includes(":")) {
    ...
    throw new Error(
      "Invalid address format. Expected an IPv4 or IPv6 address."
    );
  }
 ...
 */
export function splitAddressPort(
  address: string,
  ...
  if (address.startsWith("[")) {
    ...
    if (!match) {
      throw new Error(
        `Invalid IPv6 address format:${address}. Expected format: [<addr>]:<port>`
      );
    }
    [, addr, port] = match;
  } else {
    ...
    if ((!addr && !allowEmptyAddress) || !port) {
      throw new Error(
        `Invalid address format: ${address}. Expected format: <addr>:<port>`
      );
    }
  }
